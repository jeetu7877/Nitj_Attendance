...
function validateEmail(email) {
  ...
  if (BLOCKED_DOMAINS.has(domain)) {
    return { ok: false, err: "Disposable/temporary email domains are not allowed." };
  }
  return { ok: true, err: "" };
}
...
app.post("/register", (req, res, next) => {
  try {
    ...
    if (existing) return res.status(400).json({ detail: "Email already registered. Please login." });
...
app.post("/login", (req, res, next) => {
  try {
    ...
    if (!row) return res.status(401).json({ detail: "No account with this email." });
...
app.post("/forgot_password", async (req, res, next) => {
  try {
    ...
    const resp = { success: true, message: `Password reset code sent to ${email}.` };
    if (!sent && !SMTP_USER) {
...
app.post("/reset_password", (req, res, next) => {
  try {
    ...
    if (!row) return res.status(400).json({ detail: "No pending reset. Please request a new OTP." });
    if (row.attempts >= OTP_MAX_ATTEMPTS)
      return res.status(429).json({ detail: "Too many attempts. Please request a new OTP." });
    ...
    db.prepare("UPDATE users SET hashed_password=? WHERE id=?")
      .run(bcrypt.hashSync(new_password, 10), row.uid);
...
app.post("/create_classroom", authMiddleware, (req, res, next) => {
  try {
    const { name, subject, branch, year, section, description = "", banner_color = "#1565C0" } = req.body;
    let code;
    ...
    ).run(cid, req.user.id, name, subject, branch, year, section, code, description, banner_color);
    db.prepare(
    ...
    console.log("✅ Classroom created:", name, "[" + code + "] by", req.user.name);
    res.json({
      success:      true,
      classroom_id: cid,
      code,
...
app.get("/classroom/:cid", authMiddleware, (req, res, next) => {
  try {
    ...
    if (!mem) return res.status(403).json({ detail: "You are not a member of this classroom." });
    ...
    res.json({
      ...
      upcoming_assignments: db.prepare(
...
app.delete("/classroom/:cid", authMiddleware, (req, res, next) => {
  try {
    ...
    for (const t of ["classroom_members","attendance","posts","comments",
                     "assignments","assignment_submissions","face_audit_logs"]) {
      try { db.prepare(`DELETE FROM ${t} WHERE classroom_id=?`).run(cid); } catch {}
    }
...
app.delete("/classroom/:cid/remove/:uid", authMiddleware, (req, res, next) => {
  try {
    ...
    db.prepare("DELETE FROM classroom_members WHERE classroom_id=? AND user_id=?").run(cid, uid);
    res.json({ success: true, message: "Member removed." });
  } catch (e) { next(e); }
});

// ── JOIN CLASSROOM ───────────────────────────────────────────
app.post("/join_classroom", authMiddleware, (req, res, next) => {
  try {
    const { code, roll_number = "", branch = "", year = 1, section = "", image = "" } = req.body;
    const cls = db.prepare("SELECT * FROM classrooms WHERE code=?").get((code || "").trim().toUpperCase());
    if (!cls) return res.status(404).json({ detail: "Invalid classroom code." });

    const alreadyMember = db.prepare(
      "SELECT id FROM classroom_members WHERE classroom_id=? AND user_id=?"
    ).get(cls.id, req.user.id);
    if (alreadyMember)
      return res.status(400).json({ detail: "You are already a member of this classroom." });
    ...
    if (image) {
      // TODO: call your face-recognition microservice here.
      // Example: const enc = await callFaceService(image);
      // For now, return a stub response.
      faceMsg = "⚠️ Face recognition is not configured on this server. Contact your admin.";
    }
...
// ── ADMIN FACE RESET ─────────────────────────────────────────
app.post("/admin_reset_face", authMiddleware, (req, res, next) => {
  try {
    ...
    if (!row) return res.status(404).json({ detail: "Student not found in this classroom." });

    // TODO: encode face with your face-recognition microservice.
    // For now, acknowledge but do not update the face store.
    ...
    ).run(uuidv4(), classroom_id, student_id, "ADMIN_RESET", req.user.id, notes || "Admin reset");
    pushNotif(student_id, "Face Reset", "Your face data was reset by the admin.", "warn");
...
// ── ADMIN CLEAR FACE ─────────────────────────────────────────
app.post("/admin_clear_face", authMiddleware, upload.none(), (req, res, next) => {
  try {
    const { classroom_id, student_id, notes = "" } = req.body;
    requireAdmin(classroom_id, req.user.id);
    const now = new Date().toISOString();
    ...
    db.prepare(
      "INSERT INTO face_audit_logs (id,classroom_id,student_id,action,performed_by,notes) VALUES (?,?,?,?,?,?)"
    ).run(uuidv4(), classroom_id, student_id, "ADMIN_CLEARED", req.user.id, notes || "Cleared by admin");
    pushNotif(student_id, "Face Cleared", "Your face data was cleared by the admin.", "info");
...
app.post("/recognize", authMiddleware, (req, res, next) => {
  try {
    const { classroom_id } = req.body;
    requireAdmin(classroom_id, req.user.id);
    // TODO: call your face-recognition microservice with req.body.image.
    // The microservice should return matched student IDs + confidence scores.
    // Then insert into attendance table here.
    res.json({
      results: [{
        status:  "error",
        message: "Face recognition microservice not configured. Implement TODO in /recognize.",
      }],
    });
  } catch (e) { next(e); }
});
...
app.get("/attendance/:classroomId", authMiddleware, (req, res, next) => {
  try {
    ...
    let q, params;
    if (mem.is_admin) {
      q      = "SELECT a.*, u.name AS student_name FROM attendance a JOIN users u ON a.student_id=u.id WHERE a.classroom_id=?";
      params = [classroomId];
    } else {
      q      = "SELECT a.*, u.name AS student_name FROM attendance a JOIN users u ON a.student_id=u.id WHERE a.classroom_id=? AND a.student_id=?";
      params = [classroomId, req.user.id];
    }
    if (date_filter) { q += " AND a.date=?"; params.push(date_filter); }
...
app.get("/my_assignments", authMiddleware, (req, res, next) => {
  try {
    ...
    ).all(req.user.id).map(r => r.classroom_id);

    const result = [];
    for (const cid of classIds) {
      const cr  = db.prepare("SELECT name, subject FROM classrooms WHERE id=?").get(cid);
      if (!cr) continue;
      const mem = db.prepare(
        "SELECT is_admin FROM classroom_members WHERE classroom_id=? AND user_id=?"
      ).get(cid, req.user.id);
      ...
      for (const r of db.prepare("SELECT * FROM assignments WHERE classroom_id=? ORDER BY due_date").all(cid)) {
        const a = { ...r, classroom_name: cr.name, subject: cr.subject, is_overdue: r.due_date < todayStr, is_admin: ia };
        if (!ia) {
...
// ── ASSIGNMENTS — CREATE ─────────────────────────────────────
app.post("/create_assignment", authMiddleware, upload.single("file"), (req, res, next) => {
  try {
    const { classroom_id, title, description = "", due_date } = req.body;
    requireAdmin(classroom_id, req.user.id);

    let fu = "", fn = "";
    if (req.file) { fu = `/uploads/${req.file.filename}`; fn = req.file.originalname; }

    const aid = uuidv4();
    db.prepare(
      "INSERT INTO assignments (id,classroom_id,creator_id,title,description,file_url,file_name,due_date) VALUES (?,?,?,?,?,?,?,?)"
    ).run(aid, classroom_id, req.user.id, title, description, fu, fn, due_date);

    const mems = db.prepare(
      "SELECT user_id FROM classroom_members WHERE classroom_id=? AND is_admin=0"
    ).all(classroom_id);
    const cls  = db.prepare("SELECT name FROM classrooms WHERE id=?").get(classroom_id);
    for (const m of mems) pushNotif(m.user_id, "New Assignment", `'${title}' in ${cls.name}. Due: ${due_date}`, "info");

    res.json({ success: true, assignment_id: aid, message: "Assignment created!" });
  } catch (e) { next(e); }
});
...
app.get("/assignments/:classroomId", authMiddleware, (req, res, next) => {
  try {
    ...
    const ia   = Boolean(mem.is_admin);
    const rows = db.prepare("SELECT * FROM assignments WHERE classroom_id=? ORDER BY due_date").all(classroomId);
    const result = [];
    for (const r of rows) {
...
app.put("/assignment/:aid/due_date", authMiddleware, (req, res, next) => {
  try {
    const a = db.prepare("SELECT classroom_id FROM assignments WHERE id=?").get(req.params.aid);
    if (!a) return res.status(404).json({ detail: "Assignment not found." });
    requireAdmin(a.classroom_id, req.user.id);
    db.prepare("UPDATE assignments SET due_date=? WHERE id=?").run(req.body.due_date, req.params.aid);
...
app.delete("/assignment/:aid", authMiddleware, (req, res, next) => {
  try {
    ...
    if (!a) return res.status(404).json({ detail: "Assignment not found." });
    requireAdmin(a.classroom_id, req.user.id);
    db.prepare("DELETE FROM assignment_submissions WHERE assignment_id=?").run(req.params.aid);
...
// ── ASSIGNMENTS — SUBMIT ─────────────────────────────────────
app.post("/submit_assignment", authMiddleware, upload.single("file"), (req, res, next) => {
  try {
    const { assignment_id } = req.body;
    const asgn = db.prepare("SELECT * FROM assignments WHERE id=?").get(assignment_id);
    if (!asgn) return res.status(404).json({ detail: "Assignment not found." });
    ...
    res.json({
      success: true, status,
      message: status === "submitted" ? "Submitted on time!" : "Submitted (late).",
    });
  } catch (e) { next(e); }
});
...
app.get("/submissions/:assignmentId", authMiddleware, (req, res, next) => {
  try {
    const a = db.prepare("SELECT classroom_id FROM assignments WHERE id=?").get(req.params.assignmentId);
    if (!a) return res.status(404).json({ detail: "Assignment not found." });
    requireAdmin(a.classroom_id, req.user.id);
    const rows = db.prepare(
...
app.post("/post", authMiddleware, (req, res, next) => {
  try {
    const { classroom_id, type = "announcement", title, content = "" } = req.body;
    requireAdmin(classroom_id, req.user.id);
    const pid = uuidv4();
    db.prepare(
...
app.post("/upload_material", authMiddleware, upload.single("file"), (req, res, next) => {
  try {
    ...
    res.json({ success: true, post_id: pid, file_url: fu, message: "Uploaded!" });
  } catch (e) { next(e); }
});
...
app.delete("/post/:pid", authMiddleware, (req, res, next) => {
  try {
    const post = db.prepare("SELECT classroom_id FROM posts WHERE id=?").get(req.params.pid);
...
app.post("/comment", authMiddleware, (req, res, next) => {
  try {
    ...
    db.prepare("INSERT INTO comments (id,post_id,user_id,comment) VALUES (?,?,?,?)")
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`🚀 NITJ Classroom API v10.2.0 running on http://0.0.0.0:${PORT}`);
});